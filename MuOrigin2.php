...
function telegram($type, $notification, $msg)
{
    try
    {
        ...
        if ($notification == "true")
        {
            ...
            $data = array('chat_id' => $TelegramID, 'text' => $msg);
        }
        elseif ($notification == "false")
        {
            $url = 'https://api.telegram.org/bot' . $TelegramToken . '/sendMessage';
            $data = array('chat_id' => $TelegramID, 'text' => $msg, 'disable_notification' => true);
        }
...
try
{
    ...
    $Events = array(
        ...
        array("Abyss",  "Kundun Lilith",        "18:00",    1,  1,  1,  1,  1,  1,  1,  "true"),
    ...
    {
        if (($event_Abyss == 0) && ($currentEvent[0] == "Abyss"))
        {
            continue;
        } //Si Abyss está desactivado, skipea el evento correspondiente
        if (($event_Archangel == 0) && ($currentEvent[1] == "Archangel Boss"))
        {
            continue;
        } //Si Archangel está desactivado, skipea el evento correspondiente

        $todayEvents = 0;
        if (($serverDay == "Mon") && ($currentEvent[3] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Lunes
        elseif (($serverDay == "Tue") && ($currentEvent[4] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Martes
        elseif (($serverDay == "Wed") && ($currentEvent[5] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Miercoles
        elseif (($serverDay == "Thu") && ($currentEvent[6] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Jueves
        elseif (($serverDay == "Fri") && ($currentEvent[7] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Viernes
        elseif (($serverDay == "Sat") && ($currentEvent[8] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Sabado
        elseif (($serverDay == "Sun") && ($currentEvent[9] == 1))
        {
            $todayEvents = 1;
        } //Chequea si el evento corresponde al dia Domingo
        else
        {
            $todayEvents = 0;
        }
